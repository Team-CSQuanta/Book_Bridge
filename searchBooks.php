...
elseif(isset($_GET['category'])){
    // Get the search term from the form
    $search_term1 = $_GET['category'];

    // SQL query to search for categoy by category name
    $sql1 = "SELECT * FROM global_book_collection 
            INNER JOIN book ON global_book_collection.book_id = book.book_id 
            INNER JOIN category ON book.categoryID = category.categoryID 
            WHERE category.categoryName LIKE '%$search_term1%'";

    $search_result1 = $conn->query($sql1);
